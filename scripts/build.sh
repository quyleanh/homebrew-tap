...
# ──────────────────────────────────────────────────────────────
# Nothing else in this pipeline actually exercises a poured bottle: the generated
# wrappers ship stub `test do` blocks, so a half-relocated keg sails through and
# only shows up weeks later on a real machine (an unresolved @@HOMEBREW_LIBRARY@@
# in python's _sysconfigdata is what broke pip builds). Catch it here instead.

LAST_EXIT=0

# Run a command with a soft timeout, without depending on coreutils timeout(1).
# Always returns 0 so it is safe under `set -e`; the command's status lands in
# LAST_EXIT (143 means we timed it out ourselves).
run_capped() {
...
keg_dir="$(brew --cellar 2>/dev/null)/$pkg"
[ -d "$keg_dir" ] || return 0

# 1. No text file may keep an unresolved placeholder. A .pyc counts too: it
#    cannot be rewritten in place, so the generator drops it and Python
...
while IFS= read -r f; do
  ...
  if grep -qI "" "$f" 2>/dev/null; then
    ...
    echo "  ⚠️  $pkg: placeholder inside binary ${f#"$keg_dir"/} (inert unless it is a load command)"
  fi
done < <(grep -rl "@@HOMEBREW" "$keg_dir" 2>/dev/null || true)

# 2. Executables must actually run. Only fail on the statuses that mean broken:
...
case "$pkg" in
  python@*)
    local pbin=""
    for bin in "$keg_dir"/*/bin/python3 "$keg_dir"/bin/python3; do
      [ -x "$bin" ] && { pbin="$bin"; break; }
    done
    if [ -n "$pbin" ]; then
      run_capped "$pbin" -m pip --version
      if [ "$LAST_EXIT" -ne 0 ]; then
        echo "  ❌ $pkg: python -m pip does not run (exit $LAST_EXIT)"
        failures=$((failures + 1))
      fi
    fi
    ;;
...
if [ "$failures" -gt 0 ]; then
  echo "::error::${pkg} failed post-install verification"
  return 1
fi
echo "  ✅ $pkg verified"
return 0
...
publish_package() {
  ...
  if [ -n "$json_file" ] && [ -f "$json_file" ]; then
    ...
    # A generated wrapper must carry the full placeholder pass (PREFIX, CELLAR,
    # LIBRARY, REPOSITORY, PERL + the stale .pyc purge). One generated by an older
    # generator ships an incomplete install() and silently under-relocates.
    if [ -f "$REPO_ROOT/Formula/${pkg}.rb" ] && ! grep -q "HOMEBREW_LIBRARY" "$REPO_ROOT/Formula/${pkg}.rb"; then
      echo "  ❌ Formula/${pkg}.rb was not regenerated by the current generator"
      return 1
    fi
  fi
